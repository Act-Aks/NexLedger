...
class DailyCheckWorker(
        ...
        collectFirst(budgets) { budgetList ->
            budgetList.forEach { budget ->
                if (totalExpenses >= budget.amount * 0.9) {
                    val title = if (totalExpenses >= budget.amount) {
                        "Budget exceeded!"
                    } else {
                        "Budget alert"
                    }
    ...
    private suspend fun checkGoals() {
        ...
        collectFirst(goals) { goalList ->
            goalList
                .filter { it.deadline in now..weekFromNow && it.currentAmount < it.targetAmount }
                .forEach { goal ->
                    val remaining = goal.targetAmount - goal.currentAmount
                    showNotification(
                        NotificationChannels.GOAL_REMINDERS,
                        goal.id.toInt(),
    ...
    private suspend fun checkRecurring() {
        ...
        collectFirst(recurrings) { recurringTransactions ->
            recurringTransactions
                .filter { it.nextExecution in now..tomorrow }
                .forEach { rec ->
                    showNotification(
